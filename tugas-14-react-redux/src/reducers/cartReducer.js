...
const cartReducer = (state = initState, action) => {
  ...
  if (action.type === ADD_TO_CART) {
    ...
    newItem.quantity = newItem.quantity + 1 || 1;
    const newAddedItems = state.addedItems.find(
      (item) => item.id == action.payload.id
    )
      ? state.addedItems
      : [...state.addedItems, newItem];
    return {
      ...state,
      total: calculateTotalPrice(state.addedItems),
  ...
  if (action.type === REMOVE_ITEM) {
    // Add code here
    const index = getIndex(state.addedItems, action.payload.id);
    const newAddedItem = state.addedItems;
    newAddedItem.splice(index, 1);
  ...
  if (action.type === ADD_SHIPPING) {
    ...
    return {
      ...state,
      total: calculateTotalPrice(state.addedItems) + 6,
    };
  }
